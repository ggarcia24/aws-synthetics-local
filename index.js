...
exports.executeStep = async (stepName = null, func) => {
  ...
  await this.page.screenshot({
    path: path.join(screenShotPath, `${currIdx}${name}-starting.png`),
  });
  ...
  try {
    ...
    await this.page.screenshot({
      path: path.join(screenShotPath, `${currIdx}${name}-succeeded.png`),
    });
  } catch (e) {
    log.info(e);
    log.info(`executeStep "${currIdx}${name}" failed`);
    await this.page.screenshot({
      path: path.join(screenShotPath, `${currIdx}${name}-failed.png`),
    });
    throw e;
  } finally {
    const end = Date.now();
...
exports.takeScreenshot = async (stepName, suffix) => {
  ...
  return this.page.screenshot({
    path: path.join(screenShotPath, `${currIdx}-${stepName}-${suffix}.png`),
  });
};
